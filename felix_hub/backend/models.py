...
class Order(db.Model):
    ...
    work_status = db.Column(db.String(20), default='новый')
    ...
    assignments = db.relationship('WorkOrderAssignment', back_populates='order')
    comments = db.relationship('OrderComment', back_populates='order', order_by='OrderComment.created_at.desc()')
    time_logs = db.relationship('TimeLog', back_populates='order', order_by='TimeLog.started_at.desc()')
    custom_works = db.relationship('CustomWorkItem', back_populates='order')
    custom_parts = db.relationship('CustomPartItem', back_populates='order')
